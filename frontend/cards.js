...
async function loadCatalog(){try{const r=await fetch('/cards-data.json');catalog=await r.json();}catch{catalog=[];}renderCatalog();if(catalog[0])loadCard(catalog[0]);else resetForm();}
...
function createEffectRow(effect=effectDefaults(),index){
 ...
 renderEffectFields(row,effect);
 return row;
}
function renderEffectFields(row,effect){
 const type=row.querySelector('.effect-type').value,box=row.querySelector('.effect-fields');
 let html='';
 if(type==='damage')html=`<div class="grid2"><label>Tipo danno<input class="effect-damage-type" value="${esc(effect.damage_type||'physical')}"></label><label>Critico? <select class="effect-critical"><option value="false">No</option><option value="true" ${effect.critical?'selected':''}>Sì</option></select></label></div>`;
 ...
 box.innerHTML=html;
 box.querySelectorAll('input,select').forEach(el=>el.addEventListener('input',refreshPreview));
}
...
$('cardForm').addEventListener('submit',e=>{e.preventDefault();const card=buildCard();if(!card.id||!card.name)return alert('Inserisci ID e nome.');catalog=[...catalog.filter(x=>x.id!==card.id),card];currentCard=card;renderCatalog();renderPreview(card);alert('Carta pronta nel catalogo locale. Scarica il JSON per renderla permanente.');});
