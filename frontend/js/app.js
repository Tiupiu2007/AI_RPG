...
async function handleRaceChange() {
    ...
    try {
        character.languages = await getRaceLanguages(race.name);
    } catch (error) {
        console.error("Errore lingue razza:", error);
        character.languages = [];
    }
...
function loadCharacterIntoUI() {
    ...
    if (window.aiRpgCharacterSections?.setData) {
        window.aiRpgCharacterSections.setData(character.extra);
    }
...
function updateCharacterFromUI() {
    ...
    if (window.aiRpgCharacterSections?.getData) {
        character.extra = window.aiRpgCharacterSections.getData();
    }
